#!/usr/bin/env bun
/**
 * Production E2E Test using Gate Proof
 * 
 * This script uses gateproof to test the production deployment by:
 * 1. Making HTTP requests to production endpoints
 * 2. Validating responses and content
 * 3. Demonstrating gateproof's action/assertion pattern
 * 
 * Usage:
 *   bun run demo/production-gate.ts
 * 
 * Environment variables:
 *   PRODUCTION_URL - Production URL (defaults to https://gateproof.dev)
 * 
 * Note: Browser automation (Act.browser) requires Playwright to be installed.
 * For now, this script uses Act.exec with curl to test endpoints.
 */
