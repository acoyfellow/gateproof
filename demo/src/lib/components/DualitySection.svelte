<script lang="ts">
  import { dualityContent } from '$lib/demo-content';
</script>
    ...
    <div class="grid gap-6 lg:grid-cols-2">
      <div class="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-[0_0_0_1px_rgba(255,255,255,0.03)]">
        ...
        <ul class="mt-4 space-y-3 text-sm text-white/70">
          <li>Single artifact: <span class="font-mono text-amber-200">{dualityContent.artifactName}</span></li>
          <li>Each {dualityContent.unitLabel} encodes behavior + evidence + scope</li>
          <li>Failure output is concrete and local</li>
        </ul>
        <div class="mt-5 rounded-lg bg-black/60 border border-white/10 p-4 font-mono text-xs text-white/80">
          <div>{dualityContent.unitLabel}: "{dualityContent.unitExample.title}"</div>
          <div>id: {dualityContent.unitExample.id}</div>
          <div>gate: {dualityContent.unitExample.gatePreview}</div>
        </div>
      </div>
      ...
      <div class="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-[0_0_0_1px_rgba(255,255,255,0.03)]">
        ...
        <div class="mt-5 rounded-lg bg-black/60 border border-white/10 p-4 font-mono text-xs text-white/80">
          ...
          <div>result: {dualityContent.resultValues}</div>
        </div>
      </div>
    </div>
  </div>
